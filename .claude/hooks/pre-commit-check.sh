...
if echo "$COMMAND" | grep -qE '^git commit'; then
  ...
  if ! python -m pytest tests/test_api_sites.py tests/test_regression.py tests/test_revenue_consistency.py --ignore=tests/slow --tb=short -q 2>&1; then
    echo "" >&2
    echo "❌ Tests failed! Fix the errors before committing." >&2
    echo "   Run 'python -m pytest -v' to see detailed output." >&2
