...
> Source: [Polars User Guide - Lazy API](https://docs.pola.rs/user-guide/lazy/)
...
**Module Singleton Caching** (Superseded by DataRegistry)
...
- **Note**: Prefer using `DataRegistry` singleton for new code (ADR-007)

**DataRegistry Pattern** (NEW - 2026-02-01)
- Centralized singleton for all data access: `site_scoring/data/registry.py`
- Configurable filtering via `FilterConfig` dataclass
- Standard presets: `FILTER_ML` (active, 12+ months), `FILTER_WEB` (with coordinates)
- Thread-safe with proper locking
- Usage:
  ```python
  from site_scoring.data import DataRegistry
  registry = DataRegistry()
  training_data = registry.get_training_data()  # ML-filtered
  web_data = registry.get_web_sites()           # Web-filtered
  ```

**FeatureProcessor Pattern** (NEW - 2026-02-01)
- Unified feature processing for train AND inference: `site_scoring/data/outputs/tensor.py`
- Eliminates duplication between data_loader.py and predict.py
- Serializable: `processor.save(path)` / `FeatureProcessor.load(path)`
- Handles numeric (StandardScaler), categorical (LabelEncoder), boolean (type coercion)
...
- Score normalized to 0-1 using p20/p95 percentiles (better spread, captures top performers)
...
### ADR-007: DataRegistry Singleton for Data Access
- **Decision**: Implement centralized DataRegistry pattern for all data loading
- **Date**: 2026-02-01
- **Context**: Three separate data loading implementations (data_service.py, data_transform.py, data_loader.py) evolved independently, causing:
  - Inconsistent filtering (web shows all sites, ML filters to active only)
  - Duplicated feature processing logic
  - Maintenance burden when changing data schemas
- **Solution**: `site_scoring/data/registry.py` provides singleton with configurable FilterConfig
- **Files**:
  - `site_scoring/data/registry.py` - DataRegistry singleton
  - `site_scoring/data/outputs/tensor.py` - FeatureProcessor for train/inference
- **Consequence**:
  - Single source of truth for filtering rules
  - Consistent data access across web visualization and ML training
  - Elimination of train/inference skew via unified FeatureProcessor

### ADR-008: Transformer Chain Pattern for Data Pipeline
- **Decision**: Adopt composable transformer pattern for data transformations
- **Date**: 2026-02-01
- **Context**: data_transform.py grew to 809 lines with tightly coupled load→aggregate→join→encode→transform steps
- **Solution**: Break into separate, testable transformer classes that can be chained: `Aggregator >> FeatureEngineer >> Encoder`
- **Status**: Planned (DataRegistry and FeatureProcessor implemented first as foundation)
- **Consequence**: Each transformer testable in isolation, easy to reorder/modify pipeline
...
> Source: [gower PyPI](https://pypi.org/project/gower/) | [Gower Distance Paper (1971)](https://www.jstor.org/stable/2528823)
...
> Source: [EconML CausalForestDML](https://econml.azurewebsites.net/spec/estimation/forest.html) | [Double ML Paper (Chernozhukov 2018)](https://arxiv.org/abs/1608.00060)
...
**Robustness validation** with DoWhy sensitivity analysis ([DoWhy Docs](https://www.pywhy.org/dowhy/)):
...
> Source: [CatBoost Documentation](https://catboost.ai/en/docs/) | [CatBoost Paper (Prokhorenkova 2018)](https://arxiv.org/abs/1706.09516)
...
> Source: [MAPIE Documentation](https://mapie.readthedocs.io/en/stable/) | [Conformal Prediction Tutorial](https://mapie.readthedocs.io/en/stable/theoretical_description_classification.html)
...
> Source: [DiCE Documentation](https://interpret.ml/DiCE/) | [DiCE Paper (Mothilal 2020)](https://arxiv.org/abs/1905.07697)
...
> Source: [SHAP Documentation](https://shap.readthedocs.io/en/latest/) | [SHAP Paper (Lundberg 2017)](https://arxiv.org/abs/1705.07874)
...
| Purpose | Libraries | Documentation |
|---------|-----------|---------------|
| Core ML | scikit-learn, CatBoost, XGBoost, LightGBM | [sklearn](https://scikit-learn.org/stable/), [CatBoost](https://catboost.ai/en/docs/), [XGBoost](https://xgboost.readthedocs.io/) |
| Causal inference | EconML, CausalML, DoWhy | [EconML](https://econml.azurewebsites.net/), [DoWhy](https://www.pywhy.org/dowhy/) |
| Uncertainty | MAPIE (conformal prediction) | [MAPIE](https://mapie.readthedocs.io/en/stable/) |
| Interpretability | SHAP, DiCE | [SHAP](https://shap.readthedocs.io/), [DiCE](https://interpret.ml/DiCE/) |
| Survival analysis | lifelines, scikit-survival | [lifelines](https://lifelines.readthedocs.io/) |
| Data processing | Polars | [Polars](https://docs.pola.rs/) |

```bash
# Core ML (pinned versions as of 2026-01)
pip install scikit-learn>=1.4.0 catboost==1.2.8 xgboost==3.1.3 lightgbm>=4.0.0

# Causal inference
pip install econml==0.16.0 causalml>=0.15.0 dowhy==0.14

# Uncertainty quantification
pip install mapie==1.2.0

# Interpretability
pip install shap==0.50.0 dice-ml==0.12

# Data & utilities
pip install polars==1.37.1 gower==0.1.2
```
...
The **Discovered Patterns** directly shaped our **Architectural Decisions**:

| Pattern | Led to ADR | Rationale |
|---------|------------|-----------|
| Polars 10-20x faster than Pandas | ADR-003: Polars for Data Loading | 1.47M rows requires streaming capability |
| MPS acceleration for PyTorch | ADR-002: PyTorch over TensorFlow | Native Apple Silicon support critical for local dev |
| SSE simpler than WebSocket | ADR-005: SSE for Training | One-way data flow matches training progress pattern |
| Categorical filters → server, range → client | ADR-004: Client-Side Range Filtering | Hybrid approach optimizes both accuracy and UX |

### How Architecture Enables ML Strategy

The **Architectural Decisions** create the foundation for our **ML Strategy**:

1. **Flask + Polars** → Fast data loading enables iterative ML experimentation
2. **PyTorch + MPS** → Three-model architecture feasible on local hardware
3. **SSE streaming** → Real-time training feedback enables hyperparameter tuning
4. **Client-side filtering** → Interactive site exploration informs feature engineering
...
Edge Cases & Gotchas  ──────────────────────────────────────────────┐
        ↑                                                           │
        │ feeds back learnings                                      │
        │                                                           ↓
Discovered Patterns → Architectural Decisions → ML Strategy → Code Conventions
        ↓                                                           │
Open Questions ←────────────────────────────────────────────────────┘
        ↓
Historical (when resolved)
```
...
<!-- Pending decisions and alternatives under consideration -->

### Conformal Prediction Method Selection
- **Current**: Using APS (Adaptive Prediction Sets) for guaranteed non-empty sets
- **Alternative**: RAPS produces smaller, more actionable sets but may be empty
- **Decision needed**: Should we switch to RAPS for tighter predictions and handle empty sets gracefully?

### Lookalike Scoring k-Value
- **Current**: k=10 nearest high-performers for lookalike score
- **Question**: What's the optimal k? Should it vary by market density?
- **Next step**: Run sensitivity analysis on k ∈ {5, 10, 20, 50}

### Counterfactual Generation Scale
- **Current**: ~500ms per site with genetic search
- **Challenge**: 60K sites = ~8 hours for full fleet
- **Options**:
  1. Batch processing overnight
  2. On-demand generation for selected sites only
  3. Approximate methods (prototype-based counterfactuals)

### Model Ensemble Strategy
- **Current**: Three separate models (similarity, causal, classification)
- **Alternative**: Unified model with multi-task learning
- **Tradeoff**: Separate models = easier debugging; unified = potential for shared representations

### Feature Store Integration
- **Current**: Features computed on-the-fly during training
- **Question**: Should we implement a feature store (Feast, etc.) for production?
- **Considerations**: Training/serving skew prevention, feature versioning
